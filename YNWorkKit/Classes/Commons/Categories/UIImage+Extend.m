...
+ (YNImageType)imageDetectType:(NSData *)imageData
{
    if (!imageData) {
        return YNImageTypeUnknown;
    }
    ...
    switch (magic4) {
        ...
        case XC_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return YNImageTypeICNS;
        }
            break;
        case XC_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return YNImageTypeGIF;
        }
            break;
        case XC_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == XC_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return YNImageTypePNG;
            }
        }
            break;
    ...
    switch (magic2) {
        ...
        case XC_TWO_CC('C', 'P'): { // BMP
            return YNImageTypeBMP;
        }
        case XC_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return YNImageTypeJPEG2000;
        }
    }
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) {
        return YNImageTypeJPEG;
    }
    // JP2
...
- (UIImage *)makeCircleImageWithBorderWidth:(CGFloat)borderWidth borderColor:(UIColor *)borderColor
{
    ...
    CGFloat imaheH = self.size.height + 2 * borderWidth;
...
- (UIImage *)makeCircleImage
{
    ...
    float x4 = x1;
    float y4 = y3;
    float radius = self.size.width * 0.5;
