...
@interface ObjectCacheItem : NSObject<NSCoding,NSCopying>
...
+ (instancetype)itemWithName:(NSString *)name expired:(NSTimeInterval)expired data:(id)data
{
    ...
    ObjectCacheItem *item = [[ObjectCacheItem alloc] init];
    item.name = name;
...
@implementation ObjectCacheManager
...
- (void)saveWithItem:(ObjectCacheItem *)item
{
...
- (void)validateDataSize:(ObjectCacheItem *)item
{
...
{
    [NSKeyedArchiver archiveRootObject:object toFile:[_savaDiskRootPath stringByAppendingPathComponent:[SecurityUtil md5:name]]];
}

- (id)unarchive:(NSString *)name
{
    return [NSKeyedUnarchiver unarchiveObjectWithFile:[_savaDiskRootPath stringByAppendingPathComponent:[SecurityUtil md5:name]]];
}
...
- (void)didReceiveMemoryWarning
{
    dispatch_async(self.cacheIOQueue, ^{
        for (NSString *key in self.memoryCacheKeys) {
            ObjectCacheItem *item = [self.memoryCache objectForKey:key];
            [self archive:item name:item.name];
        }
...
{
    static ObjectCacheManager *_sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedClient = [[ObjectCacheManager alloc] init];
    });
...
- (id)syncQueryCacheForKey:(NSString *)name
{
    ...
    if ([self.memoryCacheKeys containsObject:name]) {
        ObjectCacheItem *item = [self.memoryCache objectForKey:name];
        if (item) {
            return item.data;
        }
    }
    NSString *filePath = [_savaDiskRootPath stringByAppendingPathComponent:[SecurityUtil md5:name]];
    NSFileManager *fm = [NSFileManager defaultManager];
    if ([fm fileExistsAtPath:filePath]) {
        NSDate *mdate = [fm attributesOfItemAtPath:filePath error:NULL][NSFileModificationDate];
        ObjectCacheItem *item = [NSKeyedUnarchiver unarchiveObjectWithFile:filePath];
...
{
    [self saveWithItem:[ObjectCacheItem itemWithName:name expired:CacheMaxAgeSeconds data:data]];
}

- (void)saveForKey:(NSString *)name object:(id<NSCoding>)data expired:(NSTimeInterval)expired
{
    [self saveWithItem:[ObjectCacheItem itemWithName:name expired:expired data:data]];
}

- (void)removeMemoryCacheForKey:(NSString *)name completion:(dispatch_block_t)completion
{
    if (!name) {
        return;
    }
...
- (void)removeCacheForKey:(NSString *)name completion:(dispatch_block_t)completion
{
    ...
    dispatch_async(_cacheIOQueue, ^{
        ...
        [self.memoryCache removeObjectForKey:name];
        NSFileManager *fm = [NSFileManager defaultManager];
        [fm removeItemAtPath:[_savaDiskRootPath stringByAppendingPathComponent:[SecurityUtil md5:name]] error:NULL];
        if (completion) {
